...
const DetailPage = (props: Props) => {
  ...
  const { data: mediaData, loading: dataLoading } = useFetch(
    `/${mediaType}/${id}`
  );
  ...
  const terms = ["Official Trailer", "Trailer", "Teaser", "Clip"];

  var index =
    data?.results.findIndex((element: Element) =>
      terms.some((term) => element.name.includes(term))
    ) ?? 0;
    if(index === -1){
      index = 0;
    }
  ...
  return (
    ...
    >
      ...
      {mediaType === "tv" && (
        <SeasonList id={id} data={mediaData?.seasons} loading={dataLoading} />
      )}
      {data?.results.length && <VideosSection data={data} loading={loading} />}
